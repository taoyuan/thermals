...
Escpos.prototype.bitImage = function (img, mode) {
  // todo using mode
  var data = img.toColumnFormat();
  this.stream.write(CMDS.ESC + "*" + chr(0) + intLowHigh(data.length, 2) + data);
};
...
Escpos.prototype.qr = function (content, options) {
  ...
  var v = chr(options.version || 3); // the same size is bigger
