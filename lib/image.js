...
function getColor(pixels, x, y) {
  ...
  if (chnum >= 3) {
    ...
    cols.alpha = chnum >= 4 ? (pixels.get(x, y, 3) >> 1) : 0x7F;
  } else {
...
Image.prototype.readImage = function (pixels) {
  ...
  for (var y = 0; y < this.height; y++) {
    for (var x = 0; x < this.width; x++) {
      ...
      var greyness = ((cols['red'] + cols['green'] + cols['blue']) / 3) >> 7;

      // 1 for black, 0 for white, taking into account transparency
      this.data[y * this.width + x] = (1 - greyness) & (cols['alpha'] >> 6);
    }
  }
};

Image.prototype.toColumnFormat = function () {
  ...
  var data = [];
  do {
    byteVal |= this.data[y * widthPixels + x] << (8 - bit);
    y++;
    bit++;
    if (y >= heightPixels) {
      y = 0;
  ...
  data = data.join('');
  if (data.length != (widthBytes * heightBytes * 8)) {
    throw new Error("Bug in toColumnFormat wrong number of bytes. Should be " + (widthBytes * heightBytes * 8) + " but was " + data.length);
  }
  return data;
};
