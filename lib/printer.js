 ...
 * @param {int} ec Error-correction level to use. One of QR_ECLEVEL_L (default), QR_ECLEVEL_M, QR_ECLEVEL_Q or QR_ECLEVEL_H. Higher error correction results in a less compact code.
 * @param {int} size Pixel size to use. Must be 1-16 (default 3)
 * @param {int} model QR code model to use. Must be one of QR_MODEL_1, QR_MODEL_2 (default) or QR_MICRO (not supported by all printers).
 */
Printer.prototype.qrCode = function (content, ec, size, model) {
  throw new Error('Not Implemented');
};
...
Printer.prototype.write = function (str) {
  this.adapter.write(this.stream, str);
  return this;
};
...
Printer.prototype.print = function (cb) {
  ...
  this.connector.write(this.stream.getContents(), cb);
